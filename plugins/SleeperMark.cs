...
{
    [Info("Sleeper Mark", "Ts3Hosting", "1.0.22")]
    ...
    public class SleeperMark : RustPlugin
    {
        ...
        void rewardplayer(BasePlayer player, BasePlayer attacker)
        {
                ...
                if (useRewards)
                {
                    ...
                    //if (useAirdrop) SpawnLoot(CrateAmount, attacker);
        ...
        private void FindPositionsleep(BasePlayer player1)
        {
            ...
            if (EventStarted)
            {
                ...
                onetimer.Destroy();
                if (!useBloon) PrintToChat(string.Format(lang.GetMessage("Location", this), eventplayer.displayName, eventplayer.transform.position.x, eventplayer.transform.position.z));
                if (useBloon) PrintToChat(string.Format(lang.GetMessage("LocationBloon", this), eventplayer.displayName, eventplayer.transform.position.x, eventplayer.transform.position.z));
                return;
            }
            ...
            foreach (BasePlayer player in BasePlayer.sleepingPlayerList.ToList())
            {
                ...
                if (pcdData.pEntity.ContainsKey(playerId) && (timeStamp > t) && !useBloon)
                {
