    ...
    class RemoverTool : RustPlugin
    {
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, object> p in cost)
                {
                    ...
                    if (ItemNameToItemID.ContainsKey(priceName))
                    {
						...
						// Edit: Justin
					    if (priceName.Equals("salvaged hammer"))
					    {
						    Item active = player.GetActiveItem();
							active.condition -= amount;
					    } 
						else 
						{
        ...
        static bool CanPay(BasePlayer player, BaseEntity TargetEntity)
        {
            ...
            foreach (KeyValuePair<string, object> p in cost)
            {
                ...
                if (ItemNameToItemID.ContainsKey(priceName))
                {
					...
					// Edits: Justin
					// This now checks current equipped item for salvaged hammer to
					// use durability of hammer instead of consuming the item.
					if (priceName.Equals("salvaged hammer"))
					{
						Item active = player.GetActiveItem();
						if (active == null) return false;
						// if currently equipped item is not the price item.
						    //Debug Messages
						    //player.ChatMessage(GetMsg("hammer condition " + active.condition, player));
						    //player.ChatMessage(GetMsg("" + c + " > " + active.condition + " = " + (c < active.condition), player));
						if (!active.info.itemid.Equals(ItemNameToItemID[priceName])) return false;
						// if currently equipped item does not have sufficient durability.
						if (amount > active.condition) return false;
					} 
					else
					// End edits
        ...
        static bool CanRemoveEntity(BasePlayer player, RemoveType removeType, BaseEntity TargetEntity, bool shouldPay, out string Reason)
        {
            ...
            if (shouldPay && removeType == RemoveType.Normal && !CanPay(player, TargetEntity))
            {
				...
				Reason = GetMsg("Couldn't use the RemoverTool: You don't have salvaged hammer equipped with enough durability.", player);
				// End Edit
