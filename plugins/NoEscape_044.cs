...
namespace Oxide.Plugins
{
    ...
    class NoEscape : RustPlugin
    {
        ...
        void OnPlayerAttack(BasePlayer attacker, HitInfo hitInfo)
        {
			...
			// @edited Justin
			// Fixed support for using other mod.
			
			//if (!hitInfo.isHeadshot) return;

            BasePlayer target = hitInfo.HitEntity as BasePlayer;
			
			//SendReply(attacker, "DEBUG: " + hitInfo.damageTypes.GetMajorityDamageType());
			//SendReply(target, "DEBUG: " + hitInfo.damageTypes.GetMajorityDamageType());
			
			//SendReply(attacker, "DEBUG: " + hitInfo.DoHitEffects);
			//SendReply(target, "DEBUG: " + hitInfo.DoHitEffects);
            
            //if (useZoneManager)
            //{
                var result = ZoneManager.CallHook("CanTrade", target);
				//SendReply(attacker, "DEBUG: " + result);
                if (result != null)
                {
                    return;
                }
				result = ZoneManager.CallHook("CanTrade", attacker);
				//SendReply(attacker, "DEBUG: " + result);
                if (result != null)
                {
                    return;
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo hitInfo)
        {
            if (blockOnDestroy)
            {
                ...
                if (damageTypes.Contains(hitInfo.damageTypes.GetMajorityDamageType().ToString()))
			    //if (IsDamageBlocking(hitInfo.damageTypes.GetMajorityDamageType()))
                    StructureAttack(entity, hitInfo.Initiator, hitInfo.WeaponPrefab.ShortPrefabName, hitInfo.HitPositionWorld);
        ...
        void StructureAttack(BaseEntity targetEntity, BaseEntity sourceEntity, string weapon, Vector3 hitPosition)
        {
            ...
            else if (!string.IsNullOrEmpty(targetID))	
            {
                ...
                if (ownerBlock) OwnerBlock(source, sourceEntity, target.Id, targetEntity.transform.position, sourceMembers);
				 
                if (raiderBlock) RaiderBlock(source, target.Id, targetEntity.transform.position, sourceMembers);
            }
        }
        ...
        bool IsDamageBlocking(Rust.DamageType dt)
        {
            switch (dt)
            {
				...
				// @edited Justin
				// Missing damage types for pvp combat block
				case Rust.DamageType.Slash:
				case Rust.DamageType.Arrow:
