...
namespace Oxide.Plugins
{
    ...
    class MagicSets : RustPlugin
    {
        ...
        List<BasePlayer> PlayersInRadius(Vector3 pos, float radius)
        {
            List<BasePlayer> inRadiusList = new List<BasePlayer>();
            foreach(BasePlayer player in BasePlayer.activePlayerList)
            {
                float updatedPosX = 0.0f, updatedPosY = 0.0f, updatedPosZ = 0.0f;
                updatedPosX = (player.transform.position.x - pos.x);
                updatedPosY = (player.transform.position.y - pos.y);
                updatedPosZ = (player.transform.position.z - pos.z);
                if (((updatedPosX < radius) && (updatedPosX > -radius)) 
                    && ((updatedPosY < radius) && (updatedPosY > -radius)) 
                    && ((updatedPosZ < radius) && (updatedPosZ > -radius)))
                { inRadiusList.Add(player); }
            }
            return inRadiusList;
        }
        ...
        private long UnixTimestamp() { return DateTimeOffset.UtcNow.ToUnixTimeSeconds(); }
        ...
        private long DateTimeToUnixTimestamp(DateTime dateTime) { return (long)(TimeZoneInfo.ConvertTimeToUtc(dateTime) - new DateTime(1970, 1, 1, 0, 0, 0, 0, System.DateTimeKind.Utc)).TotalSeconds; }
        ...
        private SetInfo SetFromName(BasePlayer player, string setname) { return Sets.FirstOrDefault(x => x.OwnerID == player.userID && x.Name == setname); }
        private SetInfo SetFromID(int id) { return Sets.FirstOrDefault(x => x.ID == id); }
        private bool SetExistsFromName(BasePlayer player, string setname) { return Sets.Any(x => x.OwnerID == player.userID && x.Name == setname); }
        private bool SetExistsFromID(int id) { return Sets.Any(x => x.ID == id); }
        ...
        private bool HasSets(BasePlayer player) { if (!player.IsConnected) { return false; } return Sets.Any(x => x.OwnerID == player.userID); }
        ...
        private int SetCount(BasePlayer player) { if(!player.IsConnected) { return 0; } return Sets.Count(x => x.OwnerID == player.userID); }
