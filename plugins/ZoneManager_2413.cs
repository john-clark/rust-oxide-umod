...
{
    [Info("ZoneManager", "Reneb / Nogrod", "2.4.13", ResourceId = 739)]
    public class ZoneManager : RustPlugin
    {
        ...
        private void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitinfo)
        {
            ...
            if (player != null)
            {
				...
				// @edited Justin
				// Players that had pvp god also had sleep god automatically.
				// PvP god now disabled if the player is offline and will depend on 
				// sleep god config option.
                if (player.IsSleeping() && !player.IsConnected)
                {
					//SendMessage(target, "" + player.IsConnected);
        ...
        private void OnNpcExitZone(Zone zone, BaseNpc entity, bool all = false)
        {
            ...
            else
            {
                foreach (var zone1 in zones)
                {
                    ...
                    entity.CancelInvoke(entity.TickAi);
                    entity.InvokeRandomized(entity.TickAi, 0.1f, 0.1f, 0.00500000035f);
                    break;
                }
